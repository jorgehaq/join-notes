...
def _load_configuration_safe(
    ...
    try:
        ...
        console.print(
            "[yellow]💡 Create config/projects.yml or use --config option[/yellow]"
        )
        return None
...
@click.option(
    "--profile", "-p", help="Specific profile to concatenate (default: first available)"
)
@click.option(
    "--output", "-o", type=click.Path(path_type=Path), help="Custom output file path"
)
@click.option(
    "--extensions",
    "-e",
...
@click.option(
    "--all-profiles", is_flag=True, help="Concatenate all profiles for the project"
)
@click.option(
    "--dry-run",
    is_flag=True,
...
def validate_config(ctx):
    ...
    console.print(
        f"Found {project_count} projects with {total_profiles} total profiles"
    )
...
def _display_projects_table(config: ProjectConfiguration):
    ...
    table = Table(
        title="Available Projects", show_header=True, header_style="bold magenta"
    )
    ...
    for project_name, project in config.projects.items():
        ...
        table.add_row(project_name, project.description or "No description", profiles)
...
def _concatenate_single_profile(
    ...
    if dry_run:
        console.print(
            f"[yellow]DRY RUN:[/yellow] Would concatenate project '{project_name}'"
        )
        if profile_name:
    ...
    with Progress(
        ...
        progress.add_task("Discovering and processing files...", total=None)
    ...
    if result.success:
        console.print(
            f"[green]✓[/green] Successfully concatenated {result.total_files} files"
        )
...
def _concatenate_all_profiles(
    ...
    if not project.profiles:
        console.print(
            f"[yellow]Warning:[/yellow] No profiles defined for project '{project_name}'"
        )
        return
    ...
    for profile_name in project.profiles.keys():
        if dry_run:
            console.print(
                f"[yellow]DRY RUN:[/yellow] Would process profile '{profile_name}'"
            )
            continue
        ...
        try:
            with Progress(
                ...
                progress.add_task(f"Processing {profile_name}...", total=None)
            ...
            if result.success:
                console.print(
                    f"  ✓ {result.total_files} files → {result.output_file.name}"
                )
            else:
                console.print(
                    f"  [yellow]No files found for profile '{profile_name}'[/yellow]"
                )
