...
class YamlConfigLoader:
    ...
    def _parse_configuration(self, raw_config: dict) -> ProjectConfiguration:
        ...
        return ProjectConfiguration(projects=projects, settings=settings)
...
class ConfigurationValidator:
    ...
    def _validate_profile(
        self, project_name: str, profile_name: str, profile: ProjectProfile
    ) -> List[str]:
        ...
        if not profile.pattern:
            issues.append(
                f"Project '{project_name}', profile '{profile_name}': Empty pattern"
            )

        if not profile.extensions:
            issues.append(
                f"Project '{project_name}', profile '{profile_name}': No extensions defined"
            )

        if not profile.output:
            issues.append(
                f"Project '{project_name}', profile '{profile_name}': No output filename defined"
            )

        # Validate pattern paths exist (more robust checking)
        issues.extend(
            self._validate_pattern_path(project_name, profile_name, profile.pattern)
        )

        # Validate not-include paths
        for exclude_path in profile.not_include:
            issues.extend(
                self._validate_exclude_path(project_name, profile_name, exclude_path)
            )

        return issues

    def _validate_pattern_path(
        self, project_name: str, profile_name: str, pattern: str
    ) -> List[str]:
    ...
    def _validate_exclude_path(
        self, project_name: str, profile_name: str, exclude_path: str
    ) -> List[str]:
...
def load_project_configuration_safe(
    config_path: Optional[Path] = None,
    ...
    try:
        ...
        print(
            f"❌ Configuration file not found: {config_path or 'config/projects.yml'}"
        )
        return None
    except Exception as e:
